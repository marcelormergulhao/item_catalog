...
if test == 0:
    u = User(id=1, name="test", provider="internal")
    ...
    c13 = Category(id=13, name="Fishing", description="Fishing is the activity of trying to catch fish. Fish are normally caught in the wild. Techniques for catching fish include hand gathering, spearing, netting, angling and trapping. Fishing may include catching aquatic animals other than fish, such as molluscs, cephalopods, crustaceans, and echinoderms. The term is not normally applied to catching farmed fish, or to aquatic mammals, such as whales where the term whaling is more appropriate.", picture="fishing.png", user=u)
    session.add(c13)
    session.commit()

    c14 = Category(id=14, name="Hiking", description="Hiking is the preferred term, in Canada and the United States, for a long, vigorous walk, usually on trails (footpaths), in the countryside, while the word walking is used for shorter, particularly urban walks. On the other hand, in the United Kingdom, and the Republic of Ireland, the word 'walking' is acceptable to describe all forms of walking, whether it is a walk in the park or backpacking in the Alps. The word hiking is also often used in the UK, along with rambling (a slightly old-fashioned term), hillwalking, and fell walking (a term mostly used for hillwalking in northern England). The term 'bushwalking' is endemic to Australia, having been adopted by the Sydney Bush Walkers club in 1927.[1] In New Zealand a long, vigorous walk or hike is called tramping.[2] It is a popular activity with numerous hiking organizations worldwide, and studies suggest that all forms of walking have health benefits.", picture="hiking.png", user=u)
    session.add(c14)
    session.commit()

    c15 = Category(id=15, name="Hockey", description="Hockey is a sport in which two teams play against each other by trying to maneuver a ball or a puck into the opponent's goal using a hockey stick. There are many types of hockey such as bandy, field hockey and ice hockey.", picture="hockey-1.png", user=u)
    session.add(c15)
    session.commit()

    c17 = Category(id=17, name="Karate", description="Karate (空手) (English: /kəˈrɑːtiː/; Japanese pronunciation: [kaɾate] (About this sound listen); Okinawan pronunciation: Ryukyuan pronunciation: [kaɽati]) is a martial art developed in the Ryukyu Kingdom. It developed from the indigenous Ryukyuan martial arts (called te (手), 'hand'; tii in Okinawan) under the influence of Chinese martial arts, particularly Fujian White Crane.[1][2] Karate is now predominantly a striking art using punching, kicking, knee strikes, elbow strikes and open-hand techniques such as knife-hands, spear-hands, and palm-heel strikes. Historically, and in some modern styles, grappling, throws, joint locks, restraints, and vital-point strikes are also taught.[3] A karate practitioner is called a karateka (空手家).", picture="karate.png", user=u)
    session.add(c17)
    session.commit()

    c18 = Category(id=18, name="Lacrosse", description="Lacrosse is a team sport played between two teams using a long-handled stick called a crosse and a lacrosse ball. Players use the head of the lacrosse stick to carry (called cradling), pass, and catch the ball in order to score by shooting the ball into the opponent's goal.", picture="lacrosse.png", user=u)
    session.add(c18)
    session.commit()
    ...
    c21 = Category(id=21, name="Rugby", description="Rugby is a game similar to football developed at Rugby School in Rugby, Warwickshire, one of many versions of football played at English public schools in the 19th century.[1] The two main types (known as codes) of rugby are rugby league and rugby union. Although rugby league initially used rugby union rules, they are now wholly separate sports.", picture="rugby.png", user=u)
    session.add(c21)
    session.commit()

    c22 = Category(id=22, name="Skating", description="Skating involves any sports or recreational activity which consists of traveling on surfaces or on ice using skates.", picture="skateboard.png", user=u)
    session.add(c22)
    session.commit()

    c23 = Category(id=23, name="Soccer", description="Association football, more commonly known as football or soccer,[a] is a team sport played between two teams of eleven players with a spherical ball. It is played by 250 million players in over 200 countries and dependencies, making it the world's most popular sport.[3][4][5][6] The game is played on a rectangular field with a goal at each end. The object of the game is to score by getting the ball into the opposing goal.", picture="soccer.png", user=u)
    session.add(c23)
    session.commit()

    c24 = Category(id=24, name="Surf", description="Surf is the wave activity in the area between the shoreline and outer limit of breakers. It may refer to a breaking wave in shallow water, upon the shore, or in the area in which waves break.", picture="surf-1.png", user=u)
    session.add(c24)
    session.commit()

    c25 = Category(id=25, name="Tennis", description="Tennis is a racket sport that can be played individually against a single opponent (singles) or between two teams of two players each (doubles). Each player uses a tennis racket that is strung with cord to strike a hollow rubber ball covered with felt over or around a net and into the opponent's court. The object of the game is to play the ball in such a way that the opponent is not able to play a valid return. The player who is unable to return the ball will not gain a point, while the opposite player will.", picture="tennis.png", user=u)
    session.add(c25)
    session.commit()
